...
| `slug` | `fjord` | `aurora` |
| `name` | Fjord | Aurora |
| `colourway` | Forest green | Snow white |
| `totalUnits` | 1 | 1 |
| `availableUnits` | 1 | 1 |
...
The basket. Presented as a one-time acquisition rather than a lease, and framed
as the only item on the site a customer may own outright. Exactly one exists,
ever. Modelled as a separate const in `lib/fleet.ts` with a `oneTimePriceEur`
rather than `monthlyRates`, and rendered in the lease configuration fieldset as
an optional one-time line item.
...
Stock photographs are downloaded into the repository rather than hotlinked, so
there is no remote-pattern configuration and no external runtime dependency.

| Path | Subject |
|---|---|
| `public/hero.jpg` | Interior scene with a blanket |
| `public/fleet/fjord.jpg` | Green fleece blanket |
| `public/fleet/aurora.jpg` | White fleece blanket |
| `public/korg.jpg` | Woven basket |

Replacing these files with real photographs at the same paths and aspect ratios
is the only change needed to swap in the actual product. All are rendered through
...
2. `vercel link`, targeting Erik's **personal** scope. The Vercel MCP surfaces
   only the EVAbits team, because a personal account is not a team; the scope is
   selected at link time from the CLI prompt. The selected scope is confirmed
   with Erik before any project is created.
